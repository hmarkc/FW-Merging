...
def evaluate_merged_model(models_finetuned, merged_param, data_path, metrics, outdir, eval_tasks):
    with torch.no_grad():
        ...
        ft_scores = utils.from_json('data/ft_score.json')
        normalized_score = []
        for data_name in eval_pred.keys():
            ...
            metrics[data_name] = 100*float(f"{ans:.4f}")
            normalized_score.append(100*float(f"{ans:.4f}") / ft_scores[data_name])
        metrics['score'] = np.mean(normalized_score)
...
def run_merge(
    ...
    args.models_finetuned = models_finetuned
    ...
    if args.data_path is not None:
        ...
        if 'step_size' in args:
            metrics['_step_size'] = args['step_size']
        if 'reg_factor' in args:
            metrics['_reg_factor'] = args['reg_factor']
    ...
    if args.data_path is not None:
        evaluate_merged_model(models_finetuned, merged_param, args.data_path, metrics, args.outdir, args.eval_tasks)
