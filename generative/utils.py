...
def load_truthfulqa(mode='test'):
    ...
    def process_choice(text):
        ...
        right_choice = string.ascii_uppercase[right_idx]    
        return right_choice
    ...
    def format_example(doc, keys):
        ...
        choices = "".join(
            [f"{key}. {doc['mc1_targets']['choices'][i]}\n" for i, key in enumerate(keys)]
        )
    ...
    data = data.map(
        lambda content: {
            'input': format_example(content, string.ascii_uppercase[:len(content['mc1_targets']['choices'])]),
...
def load_piqa(mode='test'):
    def process_choice(text):
        right_idx = text['label']
        right_choice = ['A', 'B'][right_idx]    
        return right_choice
    ...
    def format_example(doc, keys):
        ...
        question = doc["goal"].strip()
        choices = "".join(
            [f"{key}. {doc[['sol1', 'sol2'][i]]}\n" for i, key in enumerate(keys)]
        )
        prompt = f"Question: {question}\n{choices}Answer:"
        return prompt
    ...
    if mode == 'train':
        ...
        split = 'validation'
    data = datasets.load_dataset("piqa")[split].select(range(1000))
    data = data.map(
        lambda content: {
            'input': format_example(content, ['A', 'B']),
            'output': process_choice(content)
        }, 
        remove_columns=data.features,
        num_proc=os.cpu_count(),
...
def load_bbq(mode='test'):
    raw_datasets = load_dataset('lighteval/bbq_helm', 'all', split=mode, trust_remote_code=True)
...
def load_wmt(mode='test'):
    def format_example(doc):
        prompt = f"Romanian phrase: {doc['translation']['ro']}\nEnglish phrase:"
        return prompt
    data = datasets.load_dataset("wmt16", "ro-en", split='train').select(range(1000))
    data = data.map(
        lambda content: {
            'input': format_example(content),
...
def load_pubmedqa(mode='test'):
    ...
    data = datasets.load_dataset("bigbio/pubmed_qa", split='train').select(range(1000))
    data = data.map(
        lambda content: {
            'input': doc_to_text(content),
            'output': content['final_decision']
        }, 
        remove_columns=data.features,
        num_proc=os.cpu_count(),
    )        
...
#     data = datasets.load_dataset("mlabonne/CodeLlama-2-20k", split='train').select(range(1000))
#     data = data.map(
#         lambda content: {
#             'input': '',
...
def load_code(mode='test'):
    ...
    data = datasets.load_dataset("sahil2801/CodeAlpaca-20k", split='train').select(range(1000))
    prompt_input, prompt_no_input = PROMPT_DICT["prompt_input"], PROMPT_DICT["prompt_no_input"]
    data = data.map(
        lambda content: {
            'input': prompt_input.format_map(content) if content.get("input", "") != "" else prompt_no_input.format_map(content),
            'output': f"{content['output']}</s>"
        }, 
        remove_columns=data.features,
        num_proc=os.cpu_count(),
    )        
...
def load_cnn_dm(mode='test'):
    ...
    raw_datasets = raw_datasets.map(
        lambda x: {
            'input': "Summarize the following article: " + ' '.join(x['article'].replace('\n', ' ').split()[:256]) + ".",
...
def load_lukaemon_mmlu(mode="train"):
    ...
    raw_datasets = raw_datasets.map(
        lambda x: {
            'input': format_example(x, ["A", "B", "C", "D"]),
            'output': x["target"]
        },
        remove_columns=raw_datasets.features,
        num_proc=os.cpu_count(),
